...
const data = Array.from({ length: 12 }, (_, i) => ({
  ...
  launched: ['24 March 2006 at 22:30', '28 September 2008 23:15', '04 June 2010 18:45', '06 December 2020 16:17'][i % 4],
  ...
  return (
    <div className="overflow-x-auto p-4">
        ...
        <tbody className="">
          {data.map((item, idx) => (
            <tr key={idx} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4">{item.launched}</td>
              <td className="px-6 py-4">{item.location}</td>
              <td className="px-6 py-4">{item.mission}</td>
              <td className="px-6 py-4">{item.orbit}</td>
              <td className="px-6 py-4">
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusStyle[item.status]}`}>
                  {item.status}
                </span>
              </td>
              <td className="px-6 py-4">{item.rocket}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="flex items-center justify-end gap-1 mt-4 text-sm">
        ...
        <button className="px-3 py-1 border rounded-md bg-gray-200 font-semibold">1</button>
